	...
	if len(details.Debits) == len(details.MsgTx.TxIn) {
		for _, deb := range details.Debits {
			if deb.Amount.TokenType == 0 {
				fee += btcutil.Amount(deb.Amount.Value.(*token.NumToken).Val)
			}
		}
...
func totalBalances(dbtx walletdb.ReadTx, w *Wallet, m map[uint32]btcutil.Amount) error {
	...
	for i := range unspent {
		...
		if err == nil {
			_, ok := m[outputAcct]
			if ok {
				m[outputAcct] += btcutil.Amount(output.Amount.Value.(*token.NumToken).Val)
			}
		}
	}
	return nil
