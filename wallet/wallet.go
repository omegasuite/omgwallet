...
import (
	...
	"github.com/omegasuite/omgwallet/wallet/txrules"
	"github.com/omegasuite/omgwallet/walletdb"
	"github.com/omegasuite/omgwallet/walletdb/migration"
...
func (w *Wallet) syncWithChain(birthdayStamp *waddrmgr.BlockStamp) error {
	...
	if w.recoveryWindow <= 0 && !w.isDevEnv() {
...
func (w *Wallet) CalculateAccountBalances(account uint32, confirms int32) (Balances, error) {
	...
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		...
		for i := range unspent {
			...
			if output.Amount.TokenType == 0 {
				bals.Total += btcutil.Amount(output.Amount.Value.(*token.NumToken).Val)
				if output.FromCoinBase && !confirmed(int32(w.chainParams.CoinbaseMaturity),
					output.Height, syncBlock.Height) {
					bals.ImmatureReward += btcutil.Amount(output.Amount.Value.(*token.NumToken).Val)
				} else if confirmed(confirms, output.Height, syncBlock.Height) {
					bals.Spendable += btcutil.Amount(output.Amount.Value.(*token.NumToken).Val)
				}
			}
		}
		return nil
...
// ListAssets lists all unspent transaction
// outputs to addresses of a wallet and returns a lists.
//
// If confirmations is 0, all UTXOs, even those not present in a
// block (height -1), will be used to get the balance.  Otherwise,
// a UTXO must be in a block.  If confirmations is 1 or greater,
// the balance will be calculated based on how many how many blocks
// include a UTXO.
func (w *Wallet) ListAssets(detail bool) ([]wtxmgr.Asset, error) {
	balance := make([]wtxmgr.Asset, 0)
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		txmgrNs := tx.ReadBucket(wtxmgrNamespaceKey)
		var err error
		blk := w.Manager.SyncedTo()
...
func listTransactions(tx walletdb.ReadTx, details *wtxmgr.TxDetails, addrMgr *waddrmgr.Manager,
	...
	if len(details.Debits) == len(details.MsgTx.TxIn) {
		...
		for _, deb := range details.Debits {
			if deb.Amount.TokenType == 0 {
				debitTotal += btcutil.Amount(deb.Amount.Value.(*token.NumToken).Val)
			}
		}
	...
	err = walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		...
		for i := range unspent {
			...
			if err == nil {
				...
				if ok {
					if output.Amount.TokenType == 0 {
						*amt += btcutil.Amount(output.Amount.Value.(*token.NumToken).Val)
					}
				}
			}
		}
	...
	err = walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		...
		for i := range unspentOutputs {
			...
			case outputAcct == waddrmgr.ImportedAddrAccount:
				if output.Amount.TokenType == 0 {
					results[len(results)-1].AccountBalance += btcutil.Amount(output.Amount.Value.(*token.NumToken).Val)
				}
			...
			default:
				if output.Amount.TokenType == 0 {
					results[outputAcct].AccountBalance += btcutil.Amount(output.Amount.Value.(*token.NumToken).Val)
				}
			}
		}
		return nil
 ...
 */
			...
			result := &btcjson.ListUnspentResult{
				...
				TokenType:		output.Amount.TokenType,
				Confirmations: int64(confs),
				Spendable:     spendable,
			}
			if output.Amount.TokenType & 1 == 0 {
				result.Value = uint64(output.Amount.Value.(*token.NumToken).Val)
			} else {
				result.Hash = output.Amount.Value.(*token.HashToken).Hash.String()
			}
				...
				for _, cred := range detail.Credits {
					...
					if err == nil {
						...
						if cred.Amount.TokenType == 0 {
							res.TotalReceived += btcutil.Amount(cred.Amount.Value.(*token.NumToken).Val)
						}
				...
				for _, cred := range detail.Credits {
					...
					for _, a := range addrs {
						if addrStr == a.EncodeAddress() {
							if cred.Amount.TokenType == 0 {
								amount += btcutil.Amount(cred.Amount.Value.(*token.NumToken).Val)
							}
							break
						}
					}
...
func (w *Wallet) publishTransaction(tx *wire.MsgTx) (*chainhash.Hash, error) {
	...
	if rpcErr, ok := err.(*btcjson.RPCError); ok {
		rpcTxConfirmed = int(rpcErr.Code) == int(btcjson.ErrRPCTxAlreadyInChain)
	}
