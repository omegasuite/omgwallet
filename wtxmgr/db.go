...
// some portion of a key or.2-wqqqqqqqqqqqqqqqqqqqqqqqqqqqwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwww value, in which case `Field` describes the component
...
//
//   [0:X]   wire.Token (variable length depending on TokenType and Rights in the token)
//   [X+1]     Flags (1 byte)
//             0x01: Spent
//             0x02: Change
//   [X+2:X+74]  OPTIONAL Debit bucket key (72 bytes)
//             [0:32]  Spender transaction hash (32 bytes)
//             [32:36] Spender block height (4 bytes)
//             [36:68] Spender block hash (32 bytes)
//             [68:72] Spender transaction input index (4 bytes)
//
...
func fetchRawCreditAmount(v []byte) (*token.Token, error) {
	if len(v) < 9 {
		str := fmt.Sprintf("%s: short read (expected %d bytes, read %d)",
...
// credit is spent.
func fetchRawCreditAmountSpent(v []byte) (*token.Token, bool, error) {
	if len(v) < 9 {
		str := fmt.Sprintf("%s: short read (expected %d bytes, read %d)",
			bucketCredits, 9, len(v))
		return nil, false, storeError(ErrData, str, nil)
	}
	t := token.Token{}
	r := bytes.NewReader(v)
	...
	spent,err := r.ReadByte()
	return &t, spent & (1<<0) != 0, nil
}

// fetchRawCreditAmountChange returns the amount of the credit and whether the
// credit is marked as change.
func fetchRawCreditAmountChange(v []byte) (*token.Token, bool, error) {
	if len(v) < 9 {
		str := fmt.Sprintf("%s: short read (expected %d bytes, read %d)",
			bucketCredits, 9, len(v))
		return nil, false, storeError(ErrData, str, nil)
...
// amount is returned.
func spendCredit(ns walletdb.ReadWriteBucket, k []byte, spender *indexedIncidence) (*token.Token, error) {
	...
	v = newv
	v[n] |= 1 << 0
	copy(v[n+1:n+33], spender.txHash[:])
	byteOrder.PutUint32(v[n+33:n+37], uint32(spender.block.Height))
	copy(v[n+37:n+69], spender.block.Hash[:])
	byteOrder.PutUint32(v[n+69:n+73], spender.index)

	return &t, putRawCredit(ns, k, v)
}
...
func unspendRawCredit(ns walletdb.ReadWriteBucket, k []byte) (*token.Token, error) {
...
func (it *creditIterator) readElem() error {
	...
	it.elem.Spent = flg & (1<<0) != 0
	it.elem.Change = flg & (1<<1) != 0
	return nil
}
...
//
//   [0:X]   wire.Token (X bytes)
//   [X:X+72]  Credits bucket key (72 bytes)
//             [0:32]  Transaction hash (32 bytes)
//             [32:36] Block height (4 bytes)
//             [36:68] Block hash (32 bytes)
//             [68:72] Output index (4 bytes)
...
func putDebit(ns walletdb.ReadWriteBucket, txHash *chainhash.Hash, index uint32, amount token.Token, block *Block, credKey []byte) error {
...
func fetchRawUnminedCreditAmount(v []byte) (*token.Token, error) {
	if len(v) < 9 {
...
func fetchRawUnminedCreditAmountChange(v []byte) (*token.Token, bool, error) {
	if len(v) < 9 {
		str := "short unmined credit value"
		return nil, false, storeError(ErrData, str, nil)
	}
...
func createStore(ns walletdb.ReadWriteBucket) error {
	...
	if err := putVersion(ns, 1); err != nil {
		return err
	}
...
/*
	if _, err := ns.CreateBucket(bucketAssets); err != nil {
		str := "failed to create Assets bucket"
		return storeError(ErrDatabase, str, err)
	}
	if _, err := ns.CreateBucket(bucketUnspentAsset); err != nil {
		str := "failed to create Unspent Asset bucket"
		return storeError(ErrDatabase, str, err)
	}
	if _, err := ns.CreateBucket(bucketSpentAsset); err != nil {
		str := "failed to create bucketSpentAsset bucket"
		return storeError(ErrDatabase, str, err)
	}
	if _, err := ns.CreateBucket(bucketUnminedAssets); err != nil {
		str := "failed to create unmined Asset bucket"
		return storeError(ErrDatabase, str, err)
	}
	if _, err := ns.CreateBucket(bucketUnminedAssetOut); err != nil {
		str := "failed to create Unmined Output Asset bucket"
		return storeError(ErrDatabase, str, err)
	}
	if _, err := ns.CreateBucket(bucketUnminedAssetIn); err != nil {
		str := "failed to create Unmined Input Assetbucket"
		return storeError(ErrDatabase, str, err)
	}
*/
	return nil
...
/*
	if err := ns.DeleteNestedBucket(bucketAssets); err != nil {
		str := "failed to delete Assets bucket"
		return storeError(ErrDatabase, str, err)
	}
	if err := ns.DeleteNestedBucket(bucketUnspentAsset); err != nil {
		str := "failed to delete Unspent Asset bucket"
		return storeError(ErrDatabase, str, err)
	}
	if err := ns.DeleteNestedBucket(bucketSpentAsset); err != nil {
		str := "failed to delete Spent Asset bucket"
		return storeError(ErrDatabase, str, err)
	}
	if err := ns.DeleteNestedBucket(bucketUnminedAssets); err != nil {
		str := "failed to delete unmined Assets bucket"
		return storeError(ErrDatabase, str, err)
	}
	if err := ns.DeleteNestedBucket(bucketUnminedAssetOut); err != nil {
		str := "failed to delete Unmined Output Asset bucket"
		return storeError(ErrDatabase, str, err)
	}
	if err := ns.DeleteNestedBucket(bucketUnminedAssetIn); err != nil {
		str := "failed to delete Unmined input Asset bucket"
		return storeError(ErrDatabase, str, err)
	}
*/
	return nil
