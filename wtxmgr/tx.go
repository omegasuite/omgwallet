...
// Asset is the type representing a transaction output which was spent or
// is still spendable by wallet.  A UTXO is an unspent Credit, but not all
// Credits are UTXOs.
type Asset struct {
	wire.OutPoint
...
//		minedBalance = 0
	...
	for it.next() {
		...
		if amount.TokenType == 0 {
			newMinedBalance += btcutil.Amount(amount.Value.(*token.NumToken).Val)
		}
	}
	if it.err != nil {
		return it.err
...
func (s *Store) addCredit(ns walletdb.ReadWriteBucket, rec *TxRecord, block *BlockMeta, index uint32, change bool) (bool, error) {
	if block == nil {
		...
		if rec.MsgTx.TxOut[index].TokenType == 0 {
			v := valueUnminedCredit(rec.MsgTx.TxOut[index].Token, change)
			return true, putRawUnminedCredit(ns, k, v)
		} else {
			var txBuf bytes.Buffer
			txBuf.Grow(rec.MsgTx.SerializeSize() + 8)

			b := make([]byte, 8)
			byteOrder.PutUint64(b[:], uint64(rec.Received.Unix()))
			txBuf.Write(b)
			rec.MsgTx.Serialize(&txBuf)
			return true, putRawTxRecord(ns, k, txBuf.Bytes())
		}
		return false, nil
	}
	...
	txOutAmt := rec.MsgTx.TxOut[index].Token
	...
	if txOutAmt.TokenType == 0 {
		err = putMinedBalance(ns, minedBalance + btcutil.Amount(txOutAmt.Value.(*token.NumToken).Val))
		if err != nil {
			return false, err
		}
	}
	...
	for it.prev() {
		...
		for i := range b.transactions {
			...
			for i, input := range rec.MsgTx.TxIn {
				...
				if amt.TokenType == 0 {
					minedBalance += btcutil.Amount(amt.Value.(*token.NumToken).Val)
				}

				var txBuf bytes.Buffer
				txBuf.Grow(unspentVal.SerializeSize())
				unspentVal.WriteTxOut(&txBuf, 0, 0)

				err = putRawUnspent(ns, prevOutKey, txBuf.Bytes())
				if err != nil {
					return err
				}
			}
			...
			for i, output := range rec.MsgTx.TxOut {
				...
				if credKey != nil {
					if output.TokenType == 0 {
						minedBalance -= btcutil.Amount(output.Value.(*token.NumToken).Val)
					}
...
func (s *Store) Balance(ns walletdb.ReadBucket, minConf int32, syncHeight int32) (btcutil.Amount, error) {
	...
	err = ns.NestedReadBucket(bucketUnspent).ForEach(func(k, v []byte) error {
		...
		if existsRawUnminedInput(ns, k) != nil {
			...
			if amt.TokenType == 0 {
				bal -= btcutil.Amount(amt.Value.(*token.NumToken).Val)
			}
		}
		return nil
	})
		...
		for i := range block.transactions {
			...
			for i := uint32(0); i < numOuts; i++ {
				...
				if (confs < minConf || (blockchain.IsCoinBaseTx(&rec.MsgTx) &&
					confs < coinbaseMaturity)) && amt.TokenType == 0 {
					bal -= btcutil.Amount(amt.Value.(*token.NumToken).Val)
				}
			}
		}
	...
	if minConf == 0 {
		err = ns.NestedReadBucket(bucketUnminedCredits).ForEach(func(k, v []byte) error {
			...
			if err != nil || amount.TokenType != 0 {
				return err
			}
			bal += btcutil.Amount(amount.Value.(*token.NumToken).Val)
			return nil
		})
		if err != nil {
			if _, ok := err.(Error); ok {
...
// UnspentAssets returns all unspent received transaction asset (non basic coin type, i.e. tokentype != 0) outputs.
// The order is undefined.
func (s *Store) UnspentAssets(ns walletdb.ReadBucket, detail bool) ([]Asset, error) {
	var unspent []Asset
	nums := make(map[uint64]Asset, 0)
	...
	err := ns.NestedReadBucket(bucketUnspent).ForEach(func(k, v []byte) error {
		...
		txOut := rec.MsgTx.TxOut[op.Index]
		if !detail && txOut.TokenType & 3 == 0 {
			if _, ok := nums[txOut.TokenType]; !ok {
				nums[txOut.TokenType] = Asset{
					TokenType:    txOut.Token.TokenType,
					Value: 0,
				}
			}
			cred := nums[txOut.TokenType]
			cred.Value += txOut.Token.Value.(*token.NumToken).Val
			nums[txOut.TokenType] = cred
			return nil
		}
...
// Utxos returns the spendable wallet utxos given a minimum of minConf confirmations, calculated
// at a current chain height of curHeight. TokenType=0 assets not included.
//
// Utxos may return unexpected results if syncHeight is lower than the block
// height of the most recent mined transaction in the store.
func (s *Store) Utxos(ns walletdb.ReadBucket, detail bool, syncHeight int32) ([]Asset, error) {
	credits, err := s.UnspentAssets(ns, detail)
	if err != nil {
		return []Asset{}, err
	}

	assets := make([]Asset, 0, len(credits))

	for _, c := range credits {
		confs := syncHeight - c.Height + 1
		if c.Height > syncHeight || confs < 1 {
			continue
		}
