...
// account (wallet), or an error if the requested account does not
// exist.
func getAsset(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*btcjson.GetAssetCmd)
	...
	if accountName == "*" {
		assets, err = w.ListAssets(*cmd.Detail)
		if err != nil {
			return nil, err
		}
	} else {
/*
		...
		assets, err := w.ListAccountAssets(account)
		if err != nil {
			return nil, err
		}
*/
...
func getDB(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*btcjson.GetDBCmd)
	var dbtx walletdb.ReadTx
	wtxmgrNamespaceKey  := []byte("wtxmgr")

	dbtx, _ = w.Database().BeginReadTx()
	txmgrNs := dbtx.ReadBucket(wtxmgrNamespaceKey)

	result := make(map[string]string, 0)
	err := txmgrNs.NestedReadBucket([]byte(*cmd.Bucket)).ForEach(func(k, v []byte) error {
		...
		case "u":
			...
			copy(txHash[:], k)
			outidx := binary.BigEndian.Uint32(k[32:36])

			key = "txHash:" + txHash.String() + "\n" + "index:" + fmt.Sprintf("%d\n", outidx)
			...
			if (t.TokenType & 1) == 0 {
				...
				val += t.Value.(*token.HashToken).Hash.String() + "\n"
			}
			break
		}
		result[key] = val
	...
	for _, cred := range details.Credits {
		if !cred.Change {
			if cred.Amount.TokenType == 0 {
				creditTotal += btcutil.Amount(cred.Amount.Value.(*token.NumToken).Val)
			}
		}
	}
		...
		for _, tx := range details {
			for _, cred := range tx.Credits {
				...
				for _, addr := range addrs {
					...
					if ok {
						addrData.amount += btcutil.Amount(cred.Amount.Value.(*token.NumToken).Val)
